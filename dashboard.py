...
def plot_landuse_map(landuse_dict, obstacles):
    ...
    colors = {
        "Farming (crops)": "lightgreen",
        "Locality (town)": "lightcoral",
        "Fishery (trout)": "lightblue",
        "Poultry farm": "orange",
        "Data center": "purple",
        "Protected area": "darkgreen",
        "Intensive farming (rice/sugarcane)": "lightgreen",
        "Dense locality (village)": "lightcoral",
        "Shrimp farm": "lightblue",
        "Broiler poultry": "orange",
        "Textile factory": "brown",
        "Chemical plant": "red",
        "Corporate farming (corn/soy)": "lightgreen",
        "Suburb": "lightcoral",
        "Catfish farm": "lightblue",
        "CAFO (cattle)": "orange",
        "Data center cluster": "purple",
    }
    for name, props in landuse_dict.items():
        ...
        rect = plt.Rectangle(
            ...
            y2 - y1 + 1,
            x2 - x1 + 1,
            facecolor=colors.get(name, "gray"),
            alpha=0.5,
            edgecolor="black",
        )
        ax.add_patch(rect)
    ...
    for param, limit in limits.items():
        if param in df.columns:
            if param == "dissolved_oxygen_mgL":
                ...
                passed = min_ph >= limits.get("ph_min", 0) and max_ph <= limits.get(
                    "ph_max", 14
                )
            else:
                worst = df[param].max()
...
def main():
    st.set_page_config(
        page_title="Pre‑Establishment Compliance Dashboard", layout="wide"
    )
    st.title(
        "🌊 AI‑Based Autonomous Navigation for Industrial Pre‑Establishment Compliance"
    )
    st.markdown(
        "Select a country/regulation, view land use map, adjust proposed industry parameters, and run compliance simulation."
    )
    ...
    with st.sidebar:
        ...
        country = st.selectbox(
            "Country / Standard",
            list(LEGAL_LIMITS.keys()),
            index=list(LEGAL_LIMITS.keys()).index("India_CPCB"),
        )
        ...
        if limits:
            limit_df = pd.DataFrame(
                list(limits.items()), columns=["Parameter", "Limit"]
            )
        ...
        with col1:
            loc_x = st.number_input(
                "Location X (0-19)", min_value=0, max_value=19, value=18
            )
        with col2:
            loc_y = st.number_input(
                "Location Y (0-19)", min_value=0, max_value=19, value=18
            )
        ...
        phosphate = st.number_input(
            "Phosphate (µg/L)", value=0.10, step=0.01, format="%.2f"
        )
        ...
        mercury = st.number_input(
            "Mercury (µg/L)", value=0.001, step=0.001, format="%.3f"
        )

        st.markdown("---")
        evaluate_local_only = st.checkbox(
            "🔍 Evaluate only proposed location (ignore background)", value=True
        )
        st.caption(
            "If checked, compliance is based only on the cell where the new industry is placed."
        )

        from simulation.config import BASELINE

        heatmap_param = st.selectbox(
            "🗺️ Select Heatmap Parameter", list(BASELINE.keys()), index=0
        )
    ...
    if run_button:
        with st.spinner("Running autonomous navigation simulation..."):
            ...
            with col_left:
                ...
                limit_val = limits.get(
                    heatmap_param, np.max(env.params.get(heatmap_param, [10]))
                )
                fig_heat = plot_heatmap_with_path(
                    env, path, parameter=heatmap_param, limit=limit_val
                )
                st.pyplot(fig_heat)
            with col_right:
                ...
                if log_data:
                    ...
                    if evaluate_local_only:
                        local_rows = df_log[
                            (df_log["x"] == loc_x) & (df_log["y"] == loc_y)
                        ]
                        if len(local_rows) == 0:
                            st.warning(
                                f"Robot did not visit proposed location ({loc_x},{loc_y}). Falling back to full path."
                            )
                            df_analysis = df_log
                        else:
                            df_analysis = local_rows
                            st.info(
                                f"✅ Evaluating compliance **only at proposed location** ({loc_x},{loc_y})."
                            )
                    else:
                        df_analysis = df_log
                        st.info(
                            "📊 Evaluating compliance across **all visited cells** (worst‑case)."
                        )

                    compliance_df = check_compliance(
                        df_analysis.to_dict("records"), limits
                    )
                    st.dataframe(compliance_df, use_container_width=True)

                    violations = compliance_df[
                        compliance_df["Status"] == "❌ Violation"
                    ]
                    if len(violations) == 0:
                        st.success(
                            f"✅ **RECOMMENDATION:** The proposed {industry_name} at ({loc_x},{loc_y}) **CAN be established** under {country} regulation."
                        )
                    else:
                        st.error(
                            f"⚠️ **RECOMMENDATION:** The proposed {industry_name} **SHOULD NOT be established** without additional treatment. Violations: {', '.join(violations['Parameter'].values)}"
                        )
                else:
